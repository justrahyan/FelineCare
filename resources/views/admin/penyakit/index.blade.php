...
<div class="max-w-7xl mx-auto">
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
        <div>
            <h1 class="text-2xl md:text-3xl font-black text-slate-800 tracking-tight">Data Penyakit</h1>
            <p class="text-slate-500 text-xs md:text-sm mt-1">Kelola daftar penyakit kucing dan solusinya.</p>
        </div>
        <a href="{{ route('admin.penyakit.create') }}" class="w-full md:w-auto text-center bg-emerald-600 text-white px-6 py-3 rounded-xl font-bold hover:bg-emerald-700 transition shadow-lg shadow-emerald-100 cursor-pointer text-sm">
            + Tambah Penyakit
        </a>
    </div>
            ...
            <table class="w-full text-left border-collapse min-w-[600px]">
                <thead>
                    <tr class="text-[10px] uppercase text-slate-400 font-black tracking-widest bg-slate-50/50">
                        <th class="p-5 md:p-6">Kode</th>
                        <th class="p-5 md:p-6">Informasi Penyakit</th>
                ...
                <tbody class="text-slate-700 font-medium">
                    ...
                    <tr class="border-t border-slate-50 hover:bg-slate-50 transition">
                        <td class="p-5 md:p-6 font-bold text-emerald-600 text-xs md:text-sm">{{ $p->kode_penyakit }}</td>
                        <td class="p-5 md:p-6">
                            <p class="font-bold text-slate-800 text-sm md:text-base leading-snug">{{ $p->nama_penyakit }}</p>
                            <p class="text-xs text-slate-500 line-clamp-1 mt-1 font-normal">{{ $p->deskripsi }}</p>
                            <p class="text-[10px] text-emerald-600 font-bold mt-1 italic uppercase tracking-tighter">Solusi: {{ Str::limit($p->solusi, 50) }}</p>
                        </td>
                        <td class="p-5 md:p-6 text-center">
                            <div class="flex justify-center gap-2">
                                ...
                                <button type="button" onclick="openDeleteModal('{{ route('admin.penyakit.destroy', $p->id_penyakit) }}', '{{ $p->nama_penyakit }}')"
