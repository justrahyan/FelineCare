...
<div class="max-w-7xl mx-auto">
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
        <div>
            <h1 class="text-2xl md:text-3xl font-black text-slate-800 tracking-tight">Data Gejala</h1>
            <p class="text-slate-500 text-xs md:text-sm mt-1">Kelola tanda-tanda klinis diagnosa.</p>
        </div>
        <a href="{{ route('admin.gejala.create') }}" class="w-full md:w-auto text-center bg-emerald-600 text-white px-6 py-3 rounded-xl font-bold hover:bg-emerald-700 transition shadow-lg shadow-emerald-100 cursor-pointer text-sm">
            + Tambah Gejala
        </a>
    </div>
            ...
            <table class="w-full text-left border-collapse min-w-[600px]">
                <thead>
                    <tr class="text-[10px] uppercase text-slate-400 font-black tracking-widest bg-slate-50/50">
                        ...
                        <th class="p-5 md:p-6">Informasi Gejala</th>
                ...
                <tbody class="text-slate-700 font-medium">
                    ...
                    <tr class="border-t border-slate-50 hover:bg-slate-50 transition">
                        <td class="p-5 md:p-6 font-bold text-emerald-600 text-xs md:text-sm">{{ $g->kode_gejala }}</td>
                        <td class="p-5 md:p-6">
                            <p class="font-bold text-slate-800 text-sm md:text-base leading-snug">{{ $g->nama_gejala }}</p>
                            <span class="inline-block mt-1 bg-slate-100 text-slate-500 px-2 py-0.5 rounded-md text-[9px] font-black uppercase tracking-wider">{{ $g->kategori }}</span>
                        </td>
                        <td class="p-5 md:p-6 text-center">
                            <div class="flex justify-center gap-2">
                                ...
                                <button type="button" onclick="openDeleteModal('{{ route('admin.gejala.destroy', $g->id_gejala) }}', '{{ $g->nama_gejala }}')"
